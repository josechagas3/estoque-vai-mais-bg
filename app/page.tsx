...
type MigrationReport = { found: number; imported: number; updated: number; errors: string[] }
...
export default function Page() {
  ...
  async function migrateLegacyData() {
    const report: MigrationReport = { found: 0, imported: 0, updated: 0, errors: [] }
    try {
      const html = await fetch('/legacy-stock.html', { cache: 'no-store' }).then((response) => {
        if (!response.ok) throw new Error(`Arquivo legado indisponível (${response.status})`)
        return response.text()
      })
      const raw = html.match(/<script id="app-state" type="application\/json">([\s\S]*?)<\/script>/)?.[1]
      if (!raw) throw new Error('Estado antigo não encontrado.')
      const legacy = JSON.parse(raw) as { items?: Array<{ id: string; name: string; category?: string; unit?: string; min?: number | null }>; counts?: Array<{ date: string; values?: Record<string, number> }> }
      const items = legacy.items ?? []
      report.found = items.length
      const latest = [...(legacy.counts ?? [])].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())[0]
      const currentByName = new Map(products.map((product) => [normalizeName(product.name), product]))
      const supabase = createClient()
      const rows = items.map((item) => {
        const existing = currentByName.get(normalizeName(item.name))
        const current = Math.max(0, Number(latest?.values?.[item.id] ?? existing?.current ?? 0))
        const minimum = Math.max(0, Number(item.min ?? existing?.minimum ?? 0))
        return { id: existing?.id ?? item.id, name: item.name.trim(), category: item.category?.trim() || existing?.category || 'Outros', unit: item.unit?.trim() || existing?.unit || 'un', current_quantity: current, minimum_quantity: minimum, ideal_quantity: Math.max(current, existing?.ideal ?? minimum), updated_at: today() }
      })
      const { error } = await supabase.from('stock_products').upsert(rows, { onConflict: 'id' })
      if (error) throw error
      report.updated = rows.filter((row) => currentByName.has(normalizeName(row.name))).length
      report.imported = rows.length - report.updated
      setProducts(rows.map((row) => ({ id: row.id, name: row.name, category: row.category, unit: row.unit, current: row.current_quantity, minimum: row.minimum_quantity, ideal: row.ideal_quantity })))
    } catch (error) {
      report.errors.push(error instanceof Error ? error.message : 'Erro desconhecido na importação.')
    }
    setMigrationReport(report)
    setToast(report.errors.length ? 'Migração concluída com erros.' : 'Dados antigos migrados com segurança.')
  }
  ...
  return <div className="app-shell">
    ...
    <nav className={`nav ${menuOpen ? 'open' : ''}`}>{[['dashboard', 'Dashboard', LayoutDashboard], ['products', 'Produtos', Package], ['entry', 'Entrada de estoque', ArrowDownToLine], ['exit', 'Registrar saída', ArrowUpFromLine], ['history', 'Histórico', ClipboardList], ['purchase', 'Sugestão de compra', AlertTriangle]].map(([key, label, Icon]) => <button key={key as string} className={view === key ? 'active' : ''} onClick={() => go(key as string)}><Icon size={17} />{label as string}</button>)}</nav>
    <main className="content">{view === 'dashboard' && <Dashboard products={products} lowStock={lowStock} movements={movements} onNavigate={go} />}
      {view === 'products' && <><PageTitle title="Produtos" subtitle={`${products.length} produtos cadastrados`} action={<div className="title-actions"><button className="secondary" onClick={migrateLegacyData}>Importar dados antigos</button><button className="primary" onClick={() => setEditing({ id: crypto.randomUUID(), name: '', category: categories[0], unit: 'un', current: 0, minimum: 0, ideal: 0 })}><Plus size={17} /> Novo produto</button></div>} />{migrationReport && <div className="migration-report"><strong>Relatório da migração</strong><span>{migrationReport.found} encontrados</span><span>{migrationReport.imported} importados</span><span>{migrationReport.updated} atualizados</span>{migrationReport.errors.map((error) => <small className="migration-error" key={error}>{error}</small>)}</div>}<div className="search"><Search size={17} /><input placeholder="Buscar produto" value={query} onChange={(e) => setQuery(e.target.value)} /></div><div className="product-list">{filteredProducts.map((p) => <div className="product-card" key={p.id}><div><strong>{p.name}</strong><small>{p.category} · {p.unit}</small></div><div className="stock-number"><b className={p.current <= p.minimum ? 'low' : ''}>{p.current}</b><small>atual / mín. {p.minimum}</small></div><button className="icon-button" onClick={() => setEditing(p)} aria-label={`Editar ${p.name}`}>Editar</button></div>)}</div></>}
      ...
      {view === 'purchase' && <Purchase products={products} movements={weekMovements} />}
    </main>
...
function Purchase({ products, movements }: { products: Product[]; movements: Movement[] }) { const suggestions = products.map((p) => { const consumed = movements.filter((m) => m.productId === p.id && m.type === 'Saída').reduce((sum, m) => sum + m.quantity, 0); return { ...p, consumed, suggestion: Math.max(0, p.ideal - p.current) } }).filter((p) => p.suggestion > 0 || p.consumed > 0).sort((a, b) => b.suggestion - a.suggestion); return <><PageTitle title="Sugestão de compra" subtitle="Baseada no consumo dos últimos 7 dias" /><div className="purchase-note">Revise as quantidades antes de confirmar o pedido.</div><div className="purchase-list">{suggestions.length ? suggestions.map((p) => <div className="purchase-row" key={p.id}><div><strong>{p.name}</strong><small>Estoque {p.current} · consumo semanal {p.consumed}</small></div><input aria-label={`Quantidade para comprar de ${p.name}`} type="number" min="0" defaultValue={p.suggestion} /><span>{p.unit}</span></div>) : <Empty text="Nenhuma compra sugerida por enquanto." />}<button className="primary full" onClick={() => alert('Pedido confirmado para revisão.')}>Confirmar pedido</button></div></> }
